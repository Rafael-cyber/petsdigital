...
var __PRIVATE_IndexBackfiller = class {
  ...
  ne(e, t) {
    ...
    return PersistencePromise.doWhile(() => true === i && r2 > 0, () => this.localStore.indexManager.getNextCollectionGroupToUpdate(e).next((t2) => {
      if (null !== t2 && !n.has(t2)) return __PRIVATE_logDebug("IndexBackiller", `Processing collection: ${t2}`), this.re(e, t2, r2).next((e2) => {
        r2 -= e2, n.add(t2);
      });
      i = false;
    })).next(() => t - r2);
...
var __PRIVATE_TargetIndexMatcher = class {
  ...
  en(e) {
    ...
    for (; i < n.length && this.tn(n[i]); ++i) r2 = r2.add(n[i].fieldPath.canonicalString());
...
var __PRIVATE_IndexedDbIndexManager = class {
  ...
  hn(e, t, n) {
    ...
    for (const s of __PRIVATE_fieldIndexGetDirectionalSegments(e)) {
      const e2 = n[i++];
      for (const n2 of r2) if (this.Rn(t, s.fieldPath) && isArray(e2)) r2 = this.Vn(r2, s, e2);
      else {
  ...
  pn(e, t) {
    ...
    if (null != i) {
      ...
      if (isArray(s)) for (const i2 of s.arrayValue.values || []) n = n.add(new __PRIVATE_IndexEntry(t.indexId, e.key, this.In(i2), r2));
  ...
  createRange(e, t, n) {
    ...
    for (let e2 = 0; e2 < r2.length; e2 += 2) {
      if (this.bn(r2[e2], r2[e2 + 1])) return [];
...
var __PRIVATE_IndexedDbPersistence = class ___PRIVATE_IndexedDbPersistence {
  constructor(e, t, n, r2, i, s, o, _, a, u, c = 15) {
    ...
    this.Pi = (e2) => Promise.resolve(), !___PRIVATE_IndexedDbPersistence.D()) throw new FirestoreError(D2.UNIMPLEMENTED, "This platform is either missing IndexedDB or is known to have an incomplete implementation. Offline persistence has been disabled.");
  ...
  runTransaction(e, t, n) {
    ...
    return this.Ti.runTransaction(e, r2, i, (r3) => (s = new __PRIVATE_IndexedDbTransaction(r3, this.Lr ? this.Lr.next() : __PRIVATE_ListenSequence._e), "readwrite-primary" === t ? this.fi(s).next((e2) => !!e2 || this.gi(s)).next((t2) => {
      if (!t2) throw __PRIVATE_logError(`Failed to obtain primary lease for action '${e}'.`), this.isPrimary = false, this.si.enqueueRetryable(() => this.Pi(false)), new FirestoreError(D2.FAILED_PRECONDITION, v2);
      return n(s);
  ...
  Li(e) {
    return __PRIVATE_primaryClientStore(e).get("owner").next((e2) => {
      if (null !== e2 && this.Di(e2.leaseTimestampMs, 5e3) && !this.Fi(e2.ownerId) && !this.wi(e2) && !(this._i || this.allowTabSynchronization && e2.allowTabSynchronization)) throw new FirestoreError(D2.FAILED_PRECONDITION, de);
    });
  }
  /**
...
async function __PRIVATE_localStoreSaveNamedQuery(e, t, n = __PRIVATE_documentKeySet()) {
  ...
  return i.persistence.runTransaction("Save named query", "readwrite", (e2) => {
    ...
    if (r2.snapshotVersion.compareTo(s) >= 0) return i.Kr.saveNamedQuery(e2, t);
...
var Transaction$2 = class {
  ...
  async lookup(e) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0) throw new FirestoreError(D2.INVALID_ARGUMENT, "Firestore transactions require all reads to be executed before all writes.");
